...
data {
    ...
    real<lower=0, upper=1> pi;                    // Probability of extra zeros
...
model {
  // Priors
    ...
    for (n in 1:no_data) {
      if (Y[n] == 0) {
        target += log_sum_exp(neg_binomial_2_lpmf(Y[n] | mu[n], alpha), log(pi));
      } else {
        target += log1m(pi) + neg_binomial_2_lpmf(Y[n] | mu[n], alpha);
      }
    }
}
