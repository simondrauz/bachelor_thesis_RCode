...
generated quantities {
  ...
  // Conditional checks
  for (n in 1:no_data) {
    if (mu[n] > 1e9 || alpha > 1e9) {
      // Handle extreme values here, for example, set to a default value
      print("Extreme value was catched in y_pred_train generation.")
      y_pred_train[n] = 0;
    } else {
  ...
  for (n_eval in 1:no_data_eval) {
    if (mu_eval[n_eval] > 1e9 || alpha > 1e9) {
      // Handle extreme values here, for example, set to a default value
      print("Extreme value was catched in y_pred_eval_generation.")
      y_pred_eval[n_eval] = 0;
    } else {
