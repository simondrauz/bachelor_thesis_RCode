...
create_stan_data_model <- function(df, spatial_adjacency_matrix, eval_month, lag_indicator, no_knots, 
  ...
  small_lin_predictor_set <- c('vdem_v2x_accountability', 'vdem_v2xpe_exlecon', 
                               'vdem_v2xpe_exlpol', 'wdi_sm_pop_netm', 
                               'wdi_sm_pop_refg_or', 'wdi_ms_mil_xpnd_zs', 
                               'wdi_dt_oda_odat_pc_zs', 'wdi_sp_pop_totl')
  
  no_lin_covariates <- length(small_lin_predictor_set)
  
  # Initialize empty matrices for the covariate data
  covariate_data_linear <- matrix(nrow = nrow(df_train), ncol = length(small_lin_predictor_set))
  covariate_data_linear_eval <- matrix(nrow = nrow(df_eval), ncol = length(small_lin_predictor_set))
  
  # Set column names for both matrices
  colnames(covariate_data_linear) <- small_lin_predictor_set
  colnames(covariate_data_linear_eval) <- small_lin_predictor_set
  
  # Loop through each predictor
  for (i in 1:length(small_lin_predictor_set)) {
    predictor <- small_lin_predictor_set[i]
