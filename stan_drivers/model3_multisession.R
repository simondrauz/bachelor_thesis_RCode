...
samplingfunction <- function(x){
    ...
    res <- fit_model_for_eval_month(stan_model = stan_model_zinb, 
                                     df = df_raw,
                                     spatial_adjacency_matrix = spatial_adjacency_matrix,
...
# Retrieve results
cl <- makeCluster(no_chains * length(eval_months))
# Load the dplyr library (or magrittr for just the pipe) in each worker
clusterEvalQ(cl, {
  library(dplyr)
  library(rstan)
})
clusterExport(cl,c('stan_model_zinb','df_raw','spatial_adjacency_matrix', 'fit_model_for_eval_month', 'create_stan_data_model', 'fit_model', 'eval_months', 'lag_indicators', 'no_chains', 'cores_for_par'))
out <- parLapply(cl, c(1:length(eval_months)),samplingfunction)

for (i in 1:length(eval_months)) {
  assign(paste0("fit_", month.abb[i]), out[[i]])
  save(list = paste0("fit_", month.abb[i]), 
       file = paste0(stan_fit_dir, "fit_", model, "_", distribution_assumption, "_", feature_set, "_",  month.abb[i], "2018.RData"), 
       compress = TRUE)
  rm(list = paste0("fit_", month.abb[i]))  # remove from memory after saving
}
