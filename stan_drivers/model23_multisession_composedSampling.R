...
create_stan_data_model <- function(df, spatial_adjacency_matrix, eval_month, lag_indicator, no_knots, 
  ...
  # Set parameters for alpha and beta of Beta distribution of rho 
  alpha_rho <- 2
  beta_rho <- 2
  
  
  # Define the set of small linear predictors
  large_lin_predictor_set <- c('vdem_v2x_horacc', 'vdem_v2x_veracc', 
                               'vdem_v2xnp_client', 'vdem_v2x_divparctrl', 
                               'vdem_v2xpe_exlecon', 'vdem_v2xpe_exlpol', 
                               'vdem_v2xpe_exlsocgr', 'vdem_v2xpe_exlgeo', 
                               'vdem_v2xpe_exlgender', 'vdem_v2x_libdem', 
                               'wdi_sm_pop_netm', 'wdi_sm_pop_refg_or', 
                               'wdi_ms_mil_xpnd_zs', 'wdi_dt_oda_odat_pc_zs', 
                               'wdi_sp_pop_totl', 'wdi_sp_dyn_imrt_in')
  
  no_lin_covariates <- length(large_lin_predictor_set)
  
  # Initialize empty matrices for the covariate data
  covariate_data_linear <- matrix(nrow = nrow(df_train), ncol = length(large_lin_predictor_set))
  covariate_data_linear_eval <- matrix(nrow = nrow(df_eval), ncol = length(large_lin_predictor_set))
  
  # Set column names for both matrices
  colnames(covariate_data_linear) <- large_lin_predictor_set
  colnames(covariate_data_linear_eval) <- large_lin_predictor_set
  
  # Loop through each predictor
  for (i in 1:length(large_lin_predictor_set)) {
    predictor <- large_lin_predictor_set[i]
    train_covariate_column_name <- paste0(predictor, '_tlag_', lag_indicator)
    eval_covariate_column_name <- paste0(predictor, '_tlag_', lag_indicator)
    
    # Extract and assign lagged values for train and eval data
    covariate_data_linear[, i] <- as.numeric(df_train[[train_covariate_column_name]])
    covariate_data_linear_eval[, i] <- as.numeric(df_eval[[eval_covariate_column_name]])
  }
  
  # Convert to matrices if they are not already
  covariate_data_linear <- as.matrix(covariate_data_linear)
  covariate_data_linear_eval <- as.matrix(covariate_data_linear_eval)
  
  # Set regularization parameters for linear predictors
  a_tau_squared_linear = 0.001
  b_tau_squared_linear = 0.001
  ...
  stan_data <- list(
    ...
    random_walk_order = random_walk_order,
    no_lin_covariates = no_lin_covariates,
    ...
    covariate_data_linear = covariate_data_linear,
    covariate_data_linear_eval = covariate_data_linear_eval,
    ...
    a_tau_squared_linear = a_tau_squared_linear,
    b_tau_squared_linear = b_tau_squared_linear,
...
# Spatial Matrix from featur_set1 because it is identical
spatial_adjacency_matrix_df <- read.csv(paste0(data_dir_path, "adjacency_matrix_non_zero_and_actual_countries_feature_set1.csv"))
...
# Retrieve results
cl <- makeCluster((no_chains * length(eval_months))+1)
# Load the dplyr library (or magrittr for just the pipe) in each worker
clusterEvalQ(cl, {
  library(dplyr)
  library(rstan)
})
clusterExport(cl,c('stan_model','df_raw','spatial_adjacency_matrix', 'fit_model_for_eval_month', 'create_stan_data_model', 'fit_model', 'eval_months', 'lag_indicators', 'no_chains', 'cores_for_par'))
