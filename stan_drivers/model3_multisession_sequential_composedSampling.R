...
results <- foreach(task = iter(tasks, by = 'row'), .packages = c('rstan', 'dplyr')) %dopar% {
  ...
  fit <- fit_model_for_eval_month(stan_model = stan_model, 
                                  df = df_raw,
                                  spatial_adjacency_matrix = spatial_adjacency_matrix,
                                  eval_month = eval_month,
                                  lag_indicator = lag_indicator, 
...
stan_test <- create_stan_data_model(df=df_raw,
                                    spatial_adjacency_matrix = spatial_adjacency_matrix,
                                    eval_month = 457,
                                    lag_indicator = 3,
                                    no_knots = 20,
...
eval_data_part <- as.data.frame(stan_test$covariate_data_X1)

df <- df_raw
# Assuming your data frame is named 'df'

# Find rows with at least one NA
rows_with_na <- apply(df, 1, function(row) any(is.na(row)))

# Subset the data frame to only those rows
