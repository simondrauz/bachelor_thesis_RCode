...
spatial_adjacency_matrix_df <- read.csv(paste0(data_dir_path, "adjacency_matrix_non_zero_and_actual_countries_", feature_set, ".csv"))
...
df_eval <- df[df$month_id %in% eval_months, ]
df_eval <- df_eval[order(df_eval$month_id, df_eval$country_id), ]
saudi_arabia <- which(df_eval$country_id == 131)
syria <- which(df_eval$country_id == 220)
ethiopia <- which(df_eval$country_id == 57)
cameroon <- which(df_eval$country_id == 69)
spain <- which(df_eval$country_id == 46)
